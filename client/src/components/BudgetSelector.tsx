...
const BudgetSelector: React.FC = () => {
  const { availableBudgets, activeBudget, setActiveBudget, loading } = useBudget();
          ...
          <div className="py-1">
            ...
            {availableBudgets.map((budget) => (
              <button
                ...
                className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-100 flex items-center space-x-2 ${
                  activeBudget?.id === budget.id ? 'bg-blue-50 text-blue-700' : 'text-gray-900'
                }`}
              >
                <span>📊</span>
                <div className="flex-1">
                  <div className="font-medium">{budget.budget?.name || 'Orçamento Compartilhado'}</div>
                  <div className="text-xs text-gray-500">
                    Por {budget.budget?.owner?.name} • 
                    {budget.permission === 'READ' ? ' Visualização' : ' Edição'}
                  </div>
                </div>
